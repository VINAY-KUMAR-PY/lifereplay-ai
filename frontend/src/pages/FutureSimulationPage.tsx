...
interface ScenarioTableRowProps {
  scenario: FutureSimulationResult["scenarios"][number];
  bestScenario: string;
}

function ScenarioTableRow({ scenario, bestScenario }: ScenarioTableRowProps) {
  const strongest = metricExtreme(scenario.scorecard, "strongest");
  const weakest = metricExtreme(scenario.scorecard, "weakest");
  return (
    <tr className="border-t border-slate-100">
      <td className="p-4 font-black text-slate-950">
        {scenario.name}
        {scenario.name === bestScenario && (
          <span className="ml-2 inline-flex items-center gap-1 rounded bg-teal-50 px-2 py-1 text-xs text-teal-700">
            <Trophy size={13} /> Winner
          </span>
        )}
      </td>
      <td className="p-4 text-slate-600">{scenario.salaryAfterOneYear}</td>
      <td className="p-4 text-slate-600">{scenario.salaryAfterThreeYears}</td>
      <td className="p-4 font-black text-teal-700">{scenario.successProbability}%</td>
      <td className="p-4">{scenario.hiringDifficulty}</td>
      <td className="p-4">{scenario.riskLevel}</td>
      <td className="p-4 text-emerald-700">{strongest[1]} {scenario.scorecard[strongest[0]]}</td>
      <td className="p-4 text-amber-700">{weakest[1]} {scenario.scorecard[weakest[0]]}</td>
      <td className="p-4 font-black">{scenario.scorecard.overallScore}</td>
    </tr>
  );
}
...
function ScenarioDetailCard({ scenario }: ScenarioDetailCardProps) {
  return (
    <article className="space-y-5 border-t border-slate-200 pt-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-teal-700">Battle intelligence</p>
          <h2 className="mt-2 text-3xl font-black text-slate-950">{scenario.name}</h2>
        </div>
        <span className="rounded-md bg-slate-950 px-3 py-2 text-sm font-black text-white">
          {scenario.successProbability}% success probability
        </span>
      </div>
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-md border border-slate-200 bg-white p-5">
          <p className="text-xs font-black uppercase text-slate-500">Time and financial impact</p>
          <p className="mt-3 font-bold text-slate-950">{scenario.timeInvestment}</p>
          <p className="mt-2 text-sm leading-6 text-slate-600">{scenario.financialImpact}</p>
        </div>
        <div className="rounded-md border border-slate-200 bg-white p-5">
          <p className="text-xs font-black uppercase text-slate-500">Opportunity cost</p>
          <p className="mt-3 text-sm leading-6 text-slate-700">{scenario.opportunityCost}</p>
        </div>
        <div className="rounded-md border border-slate-200 bg-white p-5">
          <p className="text-xs font-black uppercase text-slate-500">Final outlook</p>
          <p className="mt-3 text-sm leading-6 text-slate-700">{scenario.finalOutlook}</p>
        </div>
      </div>
      <ScorecardPanel scorecard={scenario.scorecard} />
      <MarketIntelligencePanel market={scenario.marketIntelligence} />
      <SwotPanel swot={scenario.swot} />
      <RiskMatrixPanel risks={scenario.riskMatrix} />
      <div className="rounded-md bg-slate-950 p-5 text-white">
        <p className="text-xs font-black uppercase tracking-wide text-teal-300">Career impact</p>
        <p className="mt-3 leading-7 text-slate-200">{scenario.careerImpact}</p>
        <ul className="mt-4 grid gap-2 md:grid-cols-3">
          {scenario.skillsRequired.map((skill) => (
            <li key={skill} className="flex gap-2 text-sm text-slate-300">
              <ArrowRight className="mt-1 h-4 w-4 flex-none text-teal-300" />{skill}
            </li>
          ))}
        </ul>
      </div>
...
export default function FutureSimulationPage() {
  ...
  const { demoMode, clearDemoData } = useDemoData();
  ...
  return <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    {demoMode && <div className="mt-6 flex items-center justify-between gap-4 rounded-md border border-teal-200 bg-teal-50 px-4 py-3 text-sm font-black text-teal-800"><span>Demo Data - AI Engineer vs Government Exams showcase loaded without saving to history.</span><button type="button" onClick={clearDemoData} className="flex-none rounded bg-teal-700 px-3 py-1 text-xs text-white transition hover:bg-teal-800">Clear demo</button></div>}
    <form onSubmit={handleSubmit} className="mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.08)] dark:border-slate-700 dark:bg-slate-900">
      ...
      <div className="p-6"><label htmlFor="simulation-profile" className="text-sm font-black text-slate-800 dark:text-slate-100">Profile and constraints</label><textarea id="simulation-profile" rows={4} maxLength={1500} value={profile} onChange={(event) => setProfile(event.target.value)} className="mt-3 w-full resize-none rounded-xl border border-slate-300 bg-white px-4 py-3 leading-7 text-slate-950 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-100 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100 dark:focus:ring-teal-900/30" />{error && <p role="alert" className="mt-3 rounded-md bg-rose-50 p-3 text-sm font-bold text-rose-700 dark:bg-rose-950/40 dark:text-rose-300">{error}</p>}<Button type="submit" disabled={loading} className="mt-5">{loading ? <Loader2 className="animate-spin" size={18} /> : <Telescope size={18} />}{loading ? "Simulating outcomes" : "Simulate future paths"}</Button></div>
    </form>
    ...
    {result && <section className="mt-8 space-y-6"><div className="flex justify-end"><Button type="button" variant="secondary" onClick={() => downloadCareerBattleReport(result)}><Download size={17} />Download Career Battle Report</Button></div><WhyRecommendation title={`${result.bestScenario} is the strongest scenario`} reasons={result.reasoning} />
      <div className="overflow-x-auto rounded-md border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900"><table className="w-full min-w-[1100px] text-left text-sm"><thead className="bg-slate-950 text-white"><tr><th className="p-4">Scenario</th><th className="p-4">1-year salary</th><th className="p-4">3-year salary</th><th className="p-4">Success</th><th className="p-4">Hiring</th><th className="p-4">Risk</th><th className="p-4">Strongest</th><th className="p-4">Weakest</th><th className="p-4">Overall</th></tr></thead><tbody>{result.scenarios.map((scenario) => <ScenarioTableRow key={scenario.name} scenario={scenario} bestScenario={result.bestScenario} />)}</tbody></table></div>
      {result.scenarios.map((scenario) => <ScenarioDetailCard key={scenario.name} scenario={scenario} />)}
    </section>}
  </div>;
}
