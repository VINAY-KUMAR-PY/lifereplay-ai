...
function GapPanel({ title, items }: { title: string; items: string[] }) {
  return <div className="rounded-md border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900"><h3 className="font-black text-slate-950 dark:text-slate-50">{title}</h3><ul className="mt-4 space-y-3 text-sm leading-6 text-slate-600 dark:text-slate-300">{items.map((item) => <li key={item} className="flex gap-2"><XCircle className="mt-1 h-4 w-4 flex-none text-rose-500" />{item}</li>)}</ul></div>;
}
...
export default function RecruiterViewPage() {
  ...
  return <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    {demoMode && <div className="mt-6 rounded-md border border-teal-200 bg-teal-50 px-4 py-3 text-sm font-black text-teal-800 dark:border-teal-800 dark:bg-teal-950/40 dark:text-teal-200">Demo Data — CSE Graduate targeting AI Engineer.</div>}
    <form onSubmit={handleSubmit} className="mt-8 grid gap-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] dark:border-slate-700 dark:bg-slate-900 lg:grid-cols-[0.38fr_1fr]">
      <div><label htmlFor="target-role" className="text-sm font-black text-slate-800 dark:text-slate-100">Target role</label><input id="target-role" value={targetRole} onChange={(event) => setTargetRole(event.target.value)} maxLength={100} className="mt-3 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-950 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-100 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100 dark:focus:ring-teal-900/30" /></div>
      <div><label htmlFor="recruiter-profile" className="text-sm font-black text-slate-800 dark:text-slate-100">Education, skills, projects, internships, and experience</label><textarea id="recruiter-profile" rows={5} maxLength={2000} value={profile} onChange={(event) => setProfile(event.target.value)} className="mt-3 w-full resize-none rounded-xl border border-slate-300 bg-white px-4 py-3 leading-7 text-slate-950 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-100 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100 dark:focus:ring-teal-900/30" /></div>
      {error && <p role="alert" className="rounded-md bg-rose-50 p-3 text-sm font-bold text-rose-700 dark:bg-rose-950/40 dark:text-rose-300 lg:col-span-2">{error}</p>}
    ...
    {result && <section className="mt-8 space-y-6"><div className="flex justify-end"><Button type="button" variant="secondary" onClick={() => downloadRecruiterIntelligenceReport(result)}><Download size={17} />Download Recruiter Report</Button></div>
      ...
      <div className="rounded-md border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900"><h3 className="font-black text-slate-950 dark:text-slate-50">Hiring probability</h3><div className="mt-5 grid gap-5 md:grid-cols-3">{[["3 months", result.hiringProbability.threeMonths], ["6 months", result.hiringProbability.sixMonths], ["12 months", result.hiringProbability.twelveMonths]].map(([label, score]) => <div key={label as string}><div className="mb-2 flex justify-between text-sm font-bold dark:text-slate-200"><span>{label as string}</span><span>{score as number}%</span></div><div className="h-3 rounded-full bg-slate-100 dark:bg-slate-800"><div className="h-full rounded-full bg-teal-500" style={{ width: `${score as number}%` }} /></div></div>)}</div></div>
